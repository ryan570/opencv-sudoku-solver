...
def pad(img, requiredW = 32, requiredH = 32):
    ...
    wRight, hBottom = dW // 2 + 1 if wLeft < dW / 2 else dW // 2 , dH // 2 + 1 if hTop < dH / 2 else dH // 2 

    img = np.pad(img, ((hTop, hBottom), (wLeft, wRight)), 'constant', constant_values=255)
...
def remove_grid(img):
    ...
    im_bw = cv2.threshold(no_grid, mean - 1.3*stdDev, 255, cv2.THRESH_BINARY)[1]
